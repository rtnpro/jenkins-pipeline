...
def helmDeploy(Map args) {
    ...
    if (args.dry_run) {
        ...
        def helmCmd = "helm upgrade --dry-run --install ${args.name} ${args.chart_dir} --namespace=${namespace}"

        args.set.each{ k, v -> helmCmd += " --set ${k}=${v}" }

        println "Helm cmd => ${helmCmd}"

        sh "${helmCmd}"
    } else {
        println "Running deployment"

        def helmCmd = "helm upgrade --install ${args.name} ${args.chart_dir} --namespace=${namespace}"
